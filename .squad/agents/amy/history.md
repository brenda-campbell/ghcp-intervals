...
- **Pre-existing startQuiz.test.ts broken by Bender's sync questions update:** The old tests don't mock `questionsContainer`, which the updated `startQuiz.ts` now imports. 6 tests fail with 500 errors. Needs separate fix (add questionsContainer to mock + provide question pool in happy-path tests).
