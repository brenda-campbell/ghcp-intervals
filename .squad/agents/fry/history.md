...
### fe-timer-presence-cleanup (2026-04-17)
- **Countdown timer per question:** Replaced elapsed count-up timer with a countdown from configurable `timerSeconds` (default 10). `GameState` extended with `timerSeconds?: number`. New `setTimer(seconds, adminUserId)` API function (`PATCH /api/game/timer`). Countdown bar uses color-coded progress (green >5s, amber >2s, red ≤2s with pulse). Timer icon from `@phosphor-icons/react` `Timer`. Auto-timeout at 0: records answer as incorrect (0 points), shows "Time's up!" banner, auto-advances after 1s. `timedOutRef` prevents double-trigger. Timer pauses during feedback phase. `QuestionPage` accepts `timerSeconds` prop passed from `App.tsx` via `getGameState()`.
- **Admin timer config:** `QuizControlSection` in AdminPanel now shows a timer selector dropdown (5, 10, 15, 20, 30, 45, 60 seconds) when quiz is stopped. Persists via `setTimer` API call. When live, shows current timer setting as label alongside question count.
